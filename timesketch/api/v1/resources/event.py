...
from timesketch.models.sketch import Event, InvestigativeQuestionConclusion
...
class EventAnnotationResource(resources.ResourceMixin, Resource):
    ...
    def _get_search_index(
        ...
        search_indices = [t.searchindex.index_name for t in sketch.active_timelines]
        ...
        for timeline in active_timelines:
            ...
            searchindex_db_id = searchindex.id
            result = self.datastore.search(
                sketch_id=sketch.id,  # Pass sketch_id if required by method
                query_string=query_string,
                query_filter={},  # Pass empty filter if required
                query_dsl="",  # Pass empty dsl if required
                indices=[index_name],  # IMPORTANT: Query only this specific index
            )
            if result:
                return index_name
